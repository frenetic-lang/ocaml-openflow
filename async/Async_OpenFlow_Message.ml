...
module MakeSerializers (M : Message) = struct
  ...
  let deserialize ?(tags : (string * string) list = []) ?(label : string = "")
    ...
    >>= function
    | `Eof _ ->
      Log.printf ~level:`Debug ~tags:tags "[%s] EOF reading header" label;
      return `Eof
    | `Ok ->
      ...
      >>= function
      | `Eof _ ->
        Log.printf ~level:`Debug ~tags:tags
          "[%s] EOF reading body (expected %d bytes)" label body_len;
        return `Eof
      | `Ok ->
        let m = M.parse hdr (Cstruct.of_string body_buf) in
        (* extra space left so read and write align in the log *)
        Log.printf ~level:`Debug ~tags:tags "[%s] read  %s hash=%s"
          label (Header.to_string hdr) (readable_md5 (ofhdr_str ^ body_buf));
        return (`Ok m)
  ...
  let serialize ?(tags : (string * string) list = []) ?(label : string = "")
    ...
    Log.printf ~level:`Debug ~tags:tags
      "[%s] wrote %s hash=%s" label (Header.to_string hdr)
